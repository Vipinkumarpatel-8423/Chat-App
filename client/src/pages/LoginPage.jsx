  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} action="" className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        <button type='submit' className='py-3 bg-gradient-to-r form-purple-400 to-violet-600 text-white rounded-md cursor-pointer'>
        ...
        <div className='flex flex-col gap-2 text-center'>
          {currState === "Sign Up" ? (
            <p className='text-sm text-gray-600'>Already i have an account?
            ...
            <p className='text-sm text-gray-600'>Don't have an account?
              <span onClick={() => setCurrState("Sign Up")} className="font-medium text-violet-500 cursor-pointer"> Sign Up</span></p>
          )}
        </div>
      </form>
    </div >
